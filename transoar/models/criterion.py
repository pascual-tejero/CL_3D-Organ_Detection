...
class TransoarCriterion(nn.Module):
    ...
    def forward(self, outputs, targets, seg_targets, dn_meta=None, num_epoch:int=-1,
        ...
        if flag_b1_ocl: # Case we have only class labels which batch size is 1
            ...
            outputs_without_aux_2["pred_seg"] = [outputs_without_aux["pred_seg"][0][1].unsqueeze(0), outputs_without_aux["pred_seg"][1][1].unsqueeze(0), outputs_without_aux["pred_seg"][2][1].unsqueeze(0)]
            outputs_without_aux_2["neck_enc_seg"] = [outputs_without_aux["neck_enc_seg"][0][1].unsqueeze(0), outputs_without_aux["neck_enc_seg"][1][1].unsqueeze(0), outputs_without_aux["neck_enc_seg"][2][1].unsqueeze(0)]
            targets_2 = [targets[1]]
